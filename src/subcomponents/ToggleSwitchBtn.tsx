  ...
  return (
    <label className=' flex items-center cursor-pointer select-none text-sm mb-5'>
        ...
        <span className='box block h-8 w-14 mx-2 rounded-full bg-gray-200'>
            <span className={` absolute left-1 top-1 flex h-6 w-6 flex-center rounded-full bg-white dark:bg-slate-600 shadow-lg transition ${ isAllOpen ? 'translate-x-full' : ''} `}></span>
        </span>
        <span>展開全部</span>
    </label>
  )
